...
final class EventException extends AbstractException
{
    ...
    {
        return new self(new ExceptionMessage('LISTENER_NOT_CALLABLE', "Event listener is not callable for event: {$eventName}"));
    }

    public static function eventDispatchFailed(string $eventName): self
    {
        return new self(new ExceptionMessage('EVENT_DISPATCH_FAILED', "Failed to dispatch event: {$eventName}"));
    }
}
